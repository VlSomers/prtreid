...
class FeatureExtractor(object):
    ...
    def __call__(self, input):
        ...
        with torch.no_grad():
            features = self.model(images, external_parts_masks=torch.ones((images.shape[0], 2, 64, 32)))
            # features = self.model(images, external_parts_masks=torch.ones((images.shape[0], 1, 64, 32)))
