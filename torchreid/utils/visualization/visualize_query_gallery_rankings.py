...
def visualize_ranking_grid(
    ...
    mask_filtering_flag = (
        qf_parts_visibility is not None or gf_parts_visibility is not None
    )
    ...
    if n_missing > 0:
        ...
        q_idx_list = np.append(
            q_idx_list, np.random.choice(remaining_idx, replace=False, size=n_missing)
        )
    elif n_missing < 0:
    ...
    print(
        "visualize_ranking_grid for dataset {}, bp {} and ids {}".format(
            dataset_name, bp_idx, q_idx_list
        )
    )
    for q_idx in q_idx_list:
        if q_idx >= len(query_dataset):
            ...
            print(
                "Invalid query index {}, using random index {} instead".format(
                    q_idx, new_q_idx
                )
            )
            q_idx = new_q_idx
        ...
        if len(gallery_topk_samples) > 0:
            ...
            print(
                "Skip ranking plot of query id {} ({}), no valid gallery available".format(
                    q_idx, qimg_path
                )
            )
...
def show_ranking_grid(
    ...
    grid_img = 255 * np.ones(
        (
            num_rows * HEIGHT
            + (num_rows + 1) * GRID_SPACING_V
            + QUERY_EXTRA_SPACING
            + TOP_MARGIN
            + BOTTOM_MARGIN,
            num_cols * WIDTH
            + (num_cols + 1) * GRID_SPACING_H
            + QUERY_EXTRA_SPACING
            + LEFT_MARGIN
            + RIGHT_MARGIN,
            3,
    ...
    filtering_str = (
        "body part filtering with threshold {}".format(
            config.model.bpbreid.masks.mask_filtering_threshold
        )
        if config.model.bpbreid.mask_filtering_testing
        else "no body part filtering"
    )
    align_top_text(
        grid_img,
        "Ranking for dataset {}, {}, pid {}, mAP {:.2f}%, rank1 {:.2f}%, loss {}, {}".format(
            dataset_name,
            config.project.job_id,
            qpid,
            mAP * 100,
            rank1 * 100,
            config.loss.part_based.name,
            filtering_str,
        ),
        pos,
        3.5,
    ...
    for col in range(1, num_cols):
        ...
        pos = (
            col * WIDTH
            + int(WIDTH / 2)
            + (col + 1) * GRID_SPACING_H
            + QUERY_EXTRA_SPACING
            + LEFT_MARGIN,
            (row) * HEIGHT
            + int(HEIGHT / 2)
            + (row + 1) * GRID_SPACING_V
            + QUERY_EXTRA_SPACING
            + TOP_MARGIN,
        )
        ...
        align_top_multi_text(
            grid_img,
            "Bp={}/{}\nMin={:.1f}\nMean={:.1f}\nMax={:.1f}".format(
                parts_visibility_count, topk + 1, min, mean, max
            ),
            pos,
            1,
            2,
    ...
    if bp_idx is not None:
        filename = "{}_{}_qidx_{}_qpid_{}_{}_part_{}.jpg".format(
            config.project.job_id,
            dataset_name,
            qidx,
            qpid,
            ntpath.basename(qimg_path),
            bp_idx,
        )
    else:
        filename = "{}_{}_qidx_{}_qpid_{}_{}.jpg".format(
            config.project.job_id, dataset_name, qidx, qpid, ntpath.basename(qimg_path)
        )
    # path = os.path.join(save_dir, filename)
    # Path(os.path.dirname(path)).mkdir(parents=True, exist_ok=True)
    # cv2.imwrite(path, grid_img)
    Logger.current_logger().add_image(
        "Ranking grid",
        filename,
...
def insert_background_line(
    grid_img, match_color, row, height, padding_top=0, padding_bottom=0
):
    alpha = 0.1
    color = (
        255 * (1 - alpha) + match_color[0] * alpha,
...
def display_sample_on_row(
    grid_img, sample, row, img_shape, mask_filtering_flag, q_parts_visibility
):
    if row == 0:
        idx, pid, camid, img_path, masks, parts_visibility = sample
    ...
    for col in range(0, bp_num + 1):
        ...
        if row == 0 and col == 0:
            ...
            align_left_multitext(
                grid_img,
                "*Id = {}*\n"
                "Visible = {}/{}".format(pid, parts_visibility.sum(), bp_num),
                pos,
                1.1,
                2,
                15,
            ...
            pos = (
                LEFT_MARGIN + GRID_SPACING_H + int(width / 2),
                (row + 1) * height
                + (row + 1) * GRID_SPACING_V
                + QUERY_EXTRA_SPACING
                + TOP_MARGIN,
            ...
            g_to_q_vis_score = (
                np.sqrt(q_parts_visibility * parts_visibility).sum() / bp_num
            )
            align_top_text(
                grid_img,
                "{}% | {:.2f}".format(int(perc(g_to_q_vis_score, 0)), dist_to_query),
                pos,
                1.2,
                2,
                10,
            ...
            align_left_multitext(
                ...
                "[{:.2f}; {:.2f}; {:.2f}]".format(
                    ...
                    (parts_visibility > 0).sum(),
                    bp_num,
                    body_parts_dist_to_query.min(),
                    body_parts_dist_to_query.mean(),
                    body_parts_dist_to_query.max(),
                    valid_body_parts_dist.min(),
                    valid_body_parts_dist.mean(),
                    valid_body_parts_dist.max(),
                ),
                pos,
                1,
                2,
            ...
            if row == 0:
                ...
                align_top_text(
                    grid_img,
                    "{}%".format(int(perc(parts_visibility[bp_idx], 0))),
                    pos,
                    0.9,
                    2,
                    10,
            ...
            if row != 0:
                ...
                thickness = (
                    3
                    if body_parts_dist_to_query.argmax() == bp_idx
                    or body_parts_dist_to_query.argmin() == bp_idx
                    else 2
                )
                align_top_text(
                    grid_img,
                    "{}% | {:.2f}".format(
                        int(perc(parts_visibility[bp_idx], 0)),
                        body_parts_dist_to_query[bp_idx],
                    ),
                    pos,
                    0.9,
                    thickness,
            ...
            mask = masks[bp_idx, :, :]
            img_with_mask_overlay = mask_overlay(
                img, mask, interpolation=cv2.INTER_CUBIC
            )
            if mask_filtering_flag:
                # match_color = GREEN if parts_visibility[bp_idx] else RED
                match_color = cmap(parts_visibility[bp_idx].item() / 3, bytes=True)[
                    0:-1
                ]  # divided by three because hsv colormap goes from red to green inside [0, 0.333]
                img_to_insert = make_border(
                    img_with_mask_overlay,
                    (int(match_color[2]), int(match_color[1]), int(match_color[0])),
                    BW,
                )
            else:
...
def align_top_multi_text(
    img, text, pos, fontScale=1.0, thickness=1, padding=4, text_color=(0, 0, 0)
):
    v_padding = 20
    text_lines = text.split("\n")
    text_line_height = cv2.getTextSize(text_lines[0], TEXT_FONT, fontScale, thickness)[
        0
    ][1]
    ...
    for i, text_line in enumerate(text_lines):
        ...
        text_line_pos = (
            int(pos[0] - (textsize[0] / 2)),
            textY + (text_line_height + v_padding) * i,
        )
...
def make_border(img, border_color, bw):
    img_b = cv2.copyMakeBorder(
        img, bw, bw, bw, bw, cv2.BORDER_CONSTANT, value=border_color
    )
