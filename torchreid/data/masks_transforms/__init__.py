...
masks_preprocess_coco = {
    "cc6": CocoToSixBodyMasks,  # TODO: remove
...
def compute_parts_num_and_names(cfg):
    ...
    if cfg.loss.name == "part_based":
        ...
        ) or cfg.model.bpbreid.masks.preprocess == "none":
            # ISP masks or no transform
            cfg.model.bpbreid.masks.parts_num = mask_config[0]
            cfg.model.bpbreid.masks.parts_names = (
                mask_config[3]
                if 3 in mask_config
                else [
                    "p{}".format(p)
                    for p in range(1, cfg.model.bpbreid.masks.parts_num + 1)
                ]
            )
        else:
